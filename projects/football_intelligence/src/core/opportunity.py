...
from .stale_engine import compare_target_to_references
from .value_engine import MarketQuote as ValueQuote, evaluate_quote
...
def score_opportunity(target: MarketQuote, model_probability: float | None, references: list[MarketQuote]) -> Opportunity:
    value = evaluate_quote(ValueQuote(
        market=target.market.value,
        selection=target.selection,
        odds=target.odds,
        model_probability=model_probability,
    ))
    stale = compare_target_to_references(target, references)
    labels: list[str] = []
    ...
    if "MODEL_VALUE_CANDIDATE" in value.flags:
        ...
        score += min(50.0, max(0.0, (value.edge or 0.0) * 500))
        explanation.append(f"model edge={value.edge:.3f}")

    if stale.stale:
        labels.append("STALE_ODDS")
        score += min(35.0, max(0.0, (stale.deviation or 0.0) * 250))
